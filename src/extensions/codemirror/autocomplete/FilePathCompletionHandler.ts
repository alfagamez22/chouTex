...
export class FilePathCompletionHandler {
	...
	private findLatexCommand(
		context: CompletionContext,
	): { command: string; partial: string; fileTypes: string } | null {
	...
	getCompletions(
		...
		switch (fileTypes) {
			...
			case 'data':
				candidates = cache.allFiles.filter((path) =>
					/\.(dataurl|toml|csv|json|yml|yaml|xml|html|txt|md|markdown|cbor)$/i.test(
						path,
					),
				);
				break;
		...
		const options = candidates
			.map((filePath) => {
				const relativePath = isCurrentlyInTypstFile
					...
					: filePathCacheService.getLatexRelativePath(
							currentFilePath,
							filePath,
						);
			...
			.sort((a, b) => {
				const aStartsWith = a.relativePath
					.toLowerCase()
					.startsWith(partial.toLowerCase());
				const bStartsWith = b.relativePath
					.toLowerCase()
					.startsWith(partial.toLowerCase());
			...
			.map(({ relativePath, fileName, fullPath }) => ({
				...
				boost:
					partial &&
					relativePath.toLowerCase().startsWith(partial.toLowerCase())
						? 10
						: 0,
			}));
		...
		const patterns = isCurrentlyInTypstFile
