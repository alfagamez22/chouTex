...
function splitGroups(entries: ToolbarEntry[]): {
	groups: ToolbarItem[][];
	tail: ToolbarEntry[];
} {
...
function createOverflowItem(): ToolbarItem {
	return {
		...
		command: (v: EditorView) => {
			const button = v.dom.querySelector(
				`.codemirror-toolbar [data-item="${OVERFLOW_KEY}"]`,
			) as HTMLElement | null;
...
function buildItems(
	...
	collapsedGroupsByView.set(
		view,
		collapsed.map((items) => ({ items })),
	);
...
export function createCollapsableToolbar(
	...
	const measureItems = (el: HTMLElement) => {
		...
		return {
			byKey,
			split:
				el.querySelector<HTMLElement>('.cm-toolbar-split')?.offsetWidth ?? 8,
	...
	const measure = () => {
		...
		const groupWidths = groups.map((g) =>
			g.reduce((sum, item) => sum + (byKey.get(item.key) ?? 32), 0),
		);
		...
		if (collapsedIdx.size !== currentCollapse) {
			...
			view.dispatch({
				effects: compartment.reconfigure(
					toolbar({ items: buildItems(entries, collapsedIdx, view) }),
				),
			});
		}
	};
