// src/extensions/choutex-busytex/BusyTeXService.ts
    ...
    private async storeWorkFiles(): Promise<void> {
        try {
            ...
            for (const [path, buffer] of Object.entries(workFiles)) {
                const storagePath = `/.choutex_src/__work${path.startsWith('/') ? path : `/${path}`}`;
        ...
        try {
            ...
            if (result.pdf && result.pdf.length > 0) {
                ...
                outputFiles.push({
                    id: nanoid(),
                    name: pdfFileName,
                    path: `/.choutex_src/__output/${pdfFileName}`,
                    type: 'file',
