...
const LaTeXCompileButton: React.FC<LaTeXCompileButtonProps> = ({
	...
	useEffect(() => {
		const handleClickOutside = (event: MouseEvent) => {
			...
			if (dropdownRef.current && !dropdownRef.current.contains(target)) {
				...
				if (portaledDropdown?.contains(target)) {
					return;
				}
	...
	const shouldNavigateToMain = useCallback(async (): Promise<boolean> => {
		...
		return false;
	}, [getSetting, getFile, selectedFileId, selectedDocId, linkedFileInfo]);
	...
	const _handleShareFormat = (checked: boolean) => {
		if (!useSharedSettings || !changeDoc) return;
				...
				<button
					...
					title={
						isCompiling
							? t('Stop Compilation') +
								' ' +
								`${useSharedSettings ? t('(F8)') : ''}`
							: isChangingEngine
								? t('Switching Engine...')
								: t('Compile LaTeX Document') +
									' ' +
									`${useSharedSettings ? t('(F9)') : ''}`
					}
				>
					{isCompiling ? (
						<StopIcon />
              ...
              {t('Share with collaborators')}
				...
				<div className='dropdown-section'>
					...
					<div
						...
						title={
							t('Clear cache and compile') +
							' ' +
							`${useSharedSettings ? t('(Shift+F9)') : ''}`
						}
					>
						<ClearCompileIcon />
						{t('Clear & Compile')}
