...
function dumpDirContent(dir) {
  ...
  for (const entry of entries) {
    const fullPath = PATH.join2(dir, entry);
    const stat = FS.stat(fullPath);
    if (FS.isDir(stat.mode)) {
      Object.assign(files, dumpDirContent(fullPath));
    } else {
      files[fullPath] = FS.readFile(fullPath, { encoding: 'binary' });
    }
  }
  return files;
}
...
self["onmessage"] = function (ev) {
    ...
    if (cmd === "compilelatex") {
      ...
      const dir = data["url"];
      try {
        const files = dumpDirContent(dir);
        const buffers = [];
        const fileData = {};
        for (const [path, content] of Object.entries(files)) {
          fileData[path] = content.buffer;
          buffers.push(content.buffer);
        }
        self.postMessage(
          { "result": "ok", "cmd": "dircontent", "dir": dir, "files": fileData },
          buffers
        );
      } catch (e) {
        self.postMessage({ "result": "failed", "cmd": "dircontent", "log": e.toString() });
      }
    }
    else {
