...
export class GiteaBackupService {
    ...
    private readonly PLUGIN_ID = 'texlyre-gitea-backup';
    ...
    async connectToRepository(
        ...
        try {
            ...
            await this.secretsContext.setSecret(
                ...
                {
                    ...scopeOptions,
                    metadata: { tokenType: 'gitea-access-token' },
                },
            );
            await this.secretsContext.setSecret(
                this.PLUGIN_ID,
            ...
            this.addActivity({
                ...
                message: t('Connected to Gitea repository: {repoName} ({branch})', {
                    repoName,
                    branch: finalBranch,
                }),
            });
    ...
    private async getGiteaCredentials(
        projectId?: string,
        ...
        try {
            ...
            console.error(t('Error retrieving Gitea credentials:'), error);
            return null;
        }
    }
    ...
    private async ensureValidCredentials(
        ...
        const credentials = await this.getGiteaCredentials(projectId);
    ...
    async synchronize(
        ...
        try {
            ...
            this.addActivity({
                type: 'backup_complete',
                message: t('Gitea sync completed successfully'),
            });
            this.status = {
                ...this.status,
                status: 'idle',
            ...
            this._handleError(error, 'backup_error', t('Gitea sync failed'));
        }
        this.notifyListeners();
    }
    ...
    async importChanges(projectId?: string, branch?: string): Promise<void> {
        ...
        try {
            ...
            let successMessage = t('Gitea import completed successfully');
            ...
            this._handleError(error, 'import_error', t('Gitea import failed'));
        }
        this.notifyListeners();
    }
    ...
    private async importProjectSafely(
        ...
        if (data.documentsMetadataSha) {
            try {
                ...
                giteaDocumentsMetadata = JSON.parse(metadataContent);
            } catch (error) {
                console.error('Failed to load documents metadata from Gitea:', error);
            }
        }

        const docMetadataById = new Map<string, any>();
        giteaDocumentsMetadata.forEach((docMetadata) => {
        ...
        for (const [docId, docData] of data.documents.entries()) {
            const giteaDocMetadata = docMetadataById.get(docId);

            const docInfo = giteaDocMetadata || {
                id: docId,
                name: `Document ${docId}`,
        ...
        if (data.filesMetadataSha) {
            try {
                ...
                giteaFilesMetadata = JSON.parse(metadataContent);
            } catch (error) {
                console.error('Failed to load files metadata from Gitea:', error);
            }
        }

        const metadataByPath = new Map<string, any>();
        const deletedFilesMetadata = new Map<string, any>();
        giteaFilesMetadata.forEach((fileMetadata) => {
        ...
        for (const [filePath, fileSha] of data.files.entries()) {
            try {
                ...
                const giteaMetadata = metadataByPath.get(filePath);

                const isBinary = giteaMetadata
                    ? giteaMetadata.isBinary
                ...
                if (giteaMetadata) {
                    fileToStore = {
                        id:
                            existingFile?.id ||
                            giteaMetadata.id ||
                            `gitea-import-${Math.random().toString(36).substring(2, 15)}`,
                        name: giteaMetadata.name,
                        path: giteaMetadata.path,
                        type: giteaMetadata.type as 'file' | 'directory',
                        lastModified: giteaMetadata.lastModified || Date.now(),
                        size:
                            giteaMetadata.size ||
                            (finalContent instanceof ArrayBuffer
                                ? finalContent.byteLength
                                : finalContent.length),
                        mimeType: giteaMetadata.mimeType,
                        isBinary: giteaMetadata.isBinary,
                        documentId: giteaMetadata.documentId,
                ...
                await fileStorageService.storeFile(fileToStore, {
                    showConflictDialog: false,
                    preserveTimestamp: !!giteaMetadata,
                });
...
export const giteaBackupService = new GiteaBackupService();