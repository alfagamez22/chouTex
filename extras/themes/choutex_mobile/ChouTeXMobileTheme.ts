// extras/themes/choutex_mobile/chouTexMobileTheme.ts
import { t } from '@/i18n';
...
const createchouTexMobileTheme = (): ThemePlugin => {
	...
	const layout: ThemeLayout = {
		id: 'choutex-mobile',
		name: 'chouTex Mobile Theme',
		containerClass: 'choutex-mobile',
	...
	const updateMobileView = (view: string) => {
		...
		localStorage.setItem('choutex-mobile-view', view);
	};
	...
	return {
		...
		applyTheme(variantId: string): boolean {
			...
			document.documentElement.setAttribute('data-theme-plugin', 'choutex-mobile');
			document.documentElement.setAttribute('data-theme-mode', theme.isDark ? 'dark' : 'light');

			const isLoggedIn = localStorage.getItem('choutex-current-user') !== null;

			if (isLoggedIn) {
				handleMobileNavigation();

				const savedView = localStorage.getItem('choutex-mobile-view');
				if (savedView) {
				...
				loginCheckInterval = setInterval(() => {
					if (localStorage.getItem('choutex-current-user') !== null) {
						if (loginCheckInterval) {
							clearInterval(loginCheckInterval);
...
const choutexMobileTheme = createchouTexMobileTheme();

window.addEventListener('beforeunload', () => {
	if (choutexMobileTheme.cleanup) {
		choutexMobileTheme.cleanup();
	}
});
...
const observer = new MutationObserver((mutations) => {
	mutations.forEach((mutation) => {
		if (mutation.type === 'attributes' && mutation.attributeName === 'data-theme-plugin') {
			...
			if (currentPlugin !== 'choutex-mobile' && choutexMobileTheme.cleanup) {
				choutexMobileTheme.cleanup();
			}
		}
	});
...
export default choutexMobileTheme;