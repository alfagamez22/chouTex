// extras/themes/choutex_wide/chouTexWideTheme.ts
import { t } from '@/i18n';
import type {
	ThemeLayout,
	ThemePlugin,
...
const createchouTexWideTheme = (): ThemePlugin => {
	let currentThemeId = 'dark';

	const layout: ThemeLayout = {
		id: 'choutex-wide',
		name: 'chouTex Wide Theme',
		containerClass: 'choutex-wide',
	...
	return {
		...
		applyTheme(variantId: string): boolean {
			...
			document.documentElement.setAttribute('data-theme-plugin', 'choutex');
			document.documentElement.setAttribute('data-theme-mode', theme.isDark ? 'dark' : 'light');
			return true;
		},
...
const choutexWideTheme = createchouTexWideTheme();

export default choutexWideTheme;
